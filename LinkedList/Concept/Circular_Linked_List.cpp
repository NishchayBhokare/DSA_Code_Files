...
class Node{
    public:
    int data;
    Node *next; //creating pointer of type Node.
...
void InsertAtTheBegining(Node * &head,int data){
    Node *ptr=head;
    while(ptr->next != head) ptr=ptr->next; //looping till last node.

    Node * temp=new Node(data); //creating new node with provided data.
    ptr->next=temp; //pointing last node to this new first node.
...
void InsertAtTheEnd(Node * &head, int data){ //taking through reference variable so that automatic changes will happen in original head.
    ...
    temp->next=ptr->next; //pointing temp's next to ptr's next.
    ptr->next=temp; //linking that ptr node to new last node.
}
...
void DeleteAtTheLast(Node *&head){ 
    ...
    Node *ptr=head;
    while(ptr->next->next != head){ //looping till ptr not points to previous node to the last node.
        ptr=ptr->next;
    }
    delete ptr->next; //deleting ptr's next i.e last node.
    ptr->next=head; //pointing ptr's next to null.
}
...
int main(){
    ...
    second->next=third;
    third->next=fourth;
    fourth->next=head; //pointing last node to head because this is circular linked list.

    traverse(head); //Traversing

// Insertion
    // InsertAtTheBegining(head,8); //Inserting node at the beginning or inserting at the head.
    // InsertInBetween(head,4,6); //Inserting at given index.
