...
void traverseFromBack(Node *last){
    while(last->next !=NULL) last=last->next;

    while(last != NULL){ //Looping till head not reach till the last node.
        cout<< last->data<<" ";
        last=last->prev; //shifting head to its next.
    } cout<<endl;
}
...
void DeleteAtTheBegining(Node *&head){
    ...
    head->prev=NULL; //pointing head's prev to null cause now this node is new head once previous node gets deleted.
...
int main(){
    ...
    head->prev=NULL; //Connnecting linked list nodes.
    head->next=second; 
    second->prev=head;
    second->next=third;
    third->prev=second;
    ...
    traverseFromBack(fourth); //Traversing R to L.

// Insertion
    // InsertAtTheBegining(head,8); //Inserting node at the beginning or inserting at the head.
    // InsertInBetween(head,3,6); //Inserting at given index.
