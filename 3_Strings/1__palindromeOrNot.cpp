...
int isPalindrome(string s)
{
    ...
    {
        if (s[i] != s[j])
            return false; // checking if not equal then simply return false.
        i++;
        j--; // else increment i and decrement j.
    }
    return true;
}
...
bool checkPalindrome(string s)
{
    ...
    while (i <= j)
    {
        ...
        if (isalnum(s[i]) == 0)
            i++; // checking whether current character is alph numberic or not. if not then just move to next character.

        else if (isalnum(s[j]) == 0)
            j--; // checking whether current character is alph numberic or not. if not then just move to next character.

        else if (lowerCase(s[i]) != lowerCase(s[j]))
        { // if we reach till here that means charcter is alphaNumeric.
            // so if ith element not equals to jth element that means string is not palindrome so return false.
            return false;
        }

        else
        { // if same then just increment i and decrement j.
            i++;
            j--;
        }
    }
