...
int subtractProductAndSum(int n) // TC is constant and space is also constant.
{
    ...
    {                       // i.e looping till not 0 digit present in n. so that we can get all degits of particular number.
        int digit = n % 10; // by modulus of 10 we will get remainder which is last digit of particular number. for eg. if n=15 then by 15 % 10 ans will be 5, which is our last most digit.
