 ...
 vector<vector<int>> nearest(vector<vector<int>>& grid) {
        ...
        while(!q.empty()){
            ...
            int col = node[1];
            int dist = node[2];
            // ans[nRow][nCol] = dist; //we can mark dist here also. instead of doing in if block.
            for(int i = 0; i<4; i++){ //check in all four directions.
                int nRow = row + rowArr[i];
                int nCol = col + colArr[i];
                
                //if celll having zero..then update it's distance and push this new cell with its distance in queue.
                if(nRow>=0 && nRow<n && nCol>=0 && nCol<m && !visited[nRow][nCol] ){
                    // && grid[nRow][nCol] == 0 -> this equal to 0 condition is not required.
