...
//Analogy- first calculate sum of given array, then calculate sum of natural number from 0 to n-2, cause if n=7, then we will calculate sum up to first 5 as, elements present in array is from 0 to 5 with additional duplicate elements so total 7 elements. that's why n-2.
//Finally deducting sum of given array minus sum of first n-2 natural number. sum of array contains additional sum of duplicate element so that will be the answer.
