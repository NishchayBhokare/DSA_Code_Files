...
vector<int> findArrayIntersection(vector<int> &arr1, int n, vector<int> &arr2, int m)
{
  ...
  while(i<n && j<m){
      ...
      if(arr1[i]==arr2[j]){ //if element is same then push in ans.and increment in i and j.
          ans.push_back(arr1[i]); 
          i++; j++;
      }
      if(arr1[i] > arr2[j]) j++;//third condition when ith element is greater than jth element, then increment j, to check for further element.
  }
   return ans;
}
