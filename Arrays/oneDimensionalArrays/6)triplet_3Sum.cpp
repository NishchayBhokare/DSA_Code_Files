...
vector<vector<int>> ans;
    ...
    for(int i=0; i<(n-2); i++){ //looping till less than n-2, cause at least there should be 2 elements to the right of i.

        if(i==0 || (arr[i] != arr[i-1])){ //to handle case of same value of i. if same then don't enter in if block, just simply increment i.
        ...
        while(low<high){//looping till low < high cause if low is greter than or equal to high then we may get again same triplet.
            
            if((arr[low] + arr[high]) < sum) low++; //if low + high is less than sum then increment low so that we will get higher value

            else if((arr[low] + arr[high]) > sum) high--;  //if low + high is greter than sum then decrement high so that we will get lower value

            else{ // condition when (arr[low] + arr[high]) == sum
                ...
                while(low<high && arr[low]==arr[low+1]) low++; //incrementing low, till low is equal to low+1. (to avoid duplicate triplet)
                while(low<high && arr[high]==arr[high-1]) high--; //decrementing high, till high is equal to high-1.(to avoid duplicate triplet)

                low++; high--; //and incrementing low, so that low will stand on next element which will not equal to low -1. similarly for high decrementing.
            }
        }
        ...
        for(int j=(i+1); j<(n-1); j++){
            for(int k=(j+1); k<n; k++){
                ...
                temp.push_back(arr[k]);
                 if(ans.size()==0 || !(binary_search(ans.begin(),ans.end(),temp))) ans.push_back(temp);
              }
            }
        }
    }
