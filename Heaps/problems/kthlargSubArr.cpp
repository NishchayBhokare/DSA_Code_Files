...
//in this example we are adding sum of subarray with two pointer and changing sum=0 when new subarray gets start.
    // Approach 1- using sorting and while loop TC- O(n2logn) , SC-O(1)
    int kthLargest(vector<int> &Arr,int N,int K){
        ...
        sort(maxSum.begin(),maxSum.end()); //sorting vector in ascending order so that we can get kth largest sum
        int largest=maxSum[maxSum.size() - K]; //so kth largest will be size - k.
        return largest; //returning largest.
    }
    

    //Approach 2- using maxheap TC-O(N2 * logn) SC- O(n);
      int kthLargest(vector<int> &Arr,int N,int K){
    ...
    //Approach 3 - most efficient approach using min heap. TC- O(N2 * log K) SC- O(k);
      int kthLargest(vector<int> &Arr,int N,int K){
