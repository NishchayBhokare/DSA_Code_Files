...
//Approach 3: instead substracting we are just adding num+ divisor till less than dividend
long long divide(long long dividend, long long divisor) 
{
    int quotient = 0;
    int flag = 1;
    if((dividend < 0 and divisor > 0) or (dividend > 0 and divisor < 0)) flag = 0;
    long long num = abs(divisor);
    while(num <= abs(dividend)){
        quotient++;
        num += abs(divisor);
    }
